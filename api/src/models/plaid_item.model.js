...
export async function getPlaidItemByPlaidItemId(userId, plaidItemId) {
  const { rows } = await query(
    ...
    select *
    from plaid_items
    where plaid_item_id = $1 and can_access_organization_resource(user_id, organization_id, $2)
    limit 1
    `,
    [plaidItemId, userId]
  );
...
export async function listActivePlaidItemsByUser(userId) {
  const { rows } = await query(
    ...
    select *
    from plaid_items
    where can_access_organization_resource(user_id, organization_id, $1) and status = 'active'
...
export async function listActivePlaidAccountsByUser(userId) {
  const { rows } = await query(
    ...
    where can_access_organization_resource(plaid_accounts.user_id, plaid_accounts.organization_id, $1)
...
export async function getPlaidAccountByExternalId(userId, plaidAccountId) {
  const { rows } = await query(
    ...
    select *
    from plaid_accounts
    where plaid_account_id = $1 and can_access_organization_resource(user_id, organization_id, $2)
    limit 1
    `,
...
export async function countActivePlaidAccountsForItem(userId, plaidItemRef) {
  const { rows } = await query(
    ...
    from plaid_accounts
    where can_access_organization_resource(user_id, organization_id, $1) and plaid_item_ref = $2 and is_active = true
    `,
    [userId, plaidItemRef]
  );
