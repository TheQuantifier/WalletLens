...
const SAFE_USER_COLUMNS = `
  ...
  CASE WHEN organization_id IS NOT NULL THEN COALESCE((SELECT custom_expense_categories FROM organizations WHERE id = users.organization_id), '[]'::jsonb) ELSE to_jsonb(custom_expense_categories) END AS custom_expense_categories,
  CASE WHEN organization_id IS NOT NULL THEN COALESCE((SELECT custom_income_categories FROM organizations WHERE id = users.organization_id), '[]'::jsonb) ELSE to_jsonb(custom_income_categories) END AS custom_income_categories,
